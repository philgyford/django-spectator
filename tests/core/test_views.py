...
class CreatorListViewTestCase(ViewTestCase):
    ...
    def test_templates(self):
        response = views.CreatorListView.as_view()(self.request)
        self.assertEqual(response.template_name[0], "spectator_core/creator_list.html")
...
class CreatorDetailViewTestCase(ViewTestCase):
    ...
    def test_response_404(self):
        "It should raise 404 if there's no Creator with that slug."
        with self.assertRaises(Http404):
            views.CreatorDetailView.as_view()(self.request, slug="nope")

    def test_templates(self):
        response = views.CreatorDetailView.as_view()(self.request, slug="9g5o8")
        self.assertEqual(
            response.template_name[0], "spectator_core/creator_detail.html"
        )
