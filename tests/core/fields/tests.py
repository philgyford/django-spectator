...
class NaturalSortFieldTestCase(TestCase):
    ...
    def test_no_change(self):
        "If name doesn't start with an article, title_sort should be identical."
        obj = TitleModel.objects.create(title="LCD Soundsystem")
        obj.refresh_from_db()
        self.assertEqual(obj.title_sort, "lcd soundsystem")

    def test_the(self):
        "If name starts with 'The' title_sort should have it at the end."
        obj = TitleModel.objects.create(title="The Long Blondes")
        obj.refresh_from_db()
        self.assertEqual(obj.title_sort, "long blondes, the")

    def test_a(self):
        "If name starts with 'A' title_sort should have it at the end."
        obj = TitleModel.objects.create(title="A Group Name")
        obj.refresh_from_db()
        self.assertEqual(obj.title_sort, "group name, a")

    def test_a_dot(self):
        "If name starts with 'A.' title_sort should have it at the start."
        obj = TitleModel.objects.create(title="A. B. Group")
        obj.refresh_from_db()
        self.assertEqual(obj.title_sort, "a. b. group")

    def test_l(self):
        "If name starts with 'l'' title_sort should have it at the end."
        obj = TitleModel.objects.create(title="L'Impératrice")
        obj.refresh_from_db()
        self.assertEqual(obj.title_sort, "impératrice, l'")
    ...
    def test_numbers(self):
        ...
        obj.refresh_from_db()
        self.assertEqual(
            obj.title_sort, "vol. 00000002 no. 00000011, november 00002004"
        )
    ...
    def test_parentheses(self):
        "If we add parentheses after a name, they should stay at the end"
        obj = TitleModel.objects.create(title="The Charlatans (1)")
        obj.refresh_from_db()
        self.assertEqual(obj.title_sort, "charlatans, the (00000001)")
...
class PersonNaturalSortFieldTestCase(TestCase):
    ...
    def test_suffix_two_words(self):
        "If name has a suffix, but only two words, name_sort should be the same"
    ...
    def test_parentheses(self):
        "If we add parentheses after a name, they should stay at the end"
        obj = PersonModel.objects.create(name="Sam Taylor (1)")
        obj.refresh_from_db()
        self.assertEqual(obj.name_sort, "taylor, sam (00000001)")
